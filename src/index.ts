...
import TrackAudioAfv, { AFVEventTypes } from "../backend/js/trackaudio-afv-wrapper";
...
ipcMain.handle(
  "audio-get-input-devices",
  (event: IpcMainEvent, apiId: string) => {
    return TrackAudioAfv.GetAudioInputDevices(apiId);
  }
);

ipcMain.handle(
  "audio-get-output-devices",
  (event: IpcMainEvent, apiId: string) => {
...
ipcMain.handle("connect", () => {
  setAudioSettings();
  return TrackAudioAfv.Connect(
    currentConfiguration.cid,
...
ipcMain.handle(
  "audio-add-frequency",
  (event: IpcMainEvent, frequency: number, callsign: string) => {
...
ipcMain.handle(
  ...
  (event: IpcMainEvent, frequency: number) => {
...
ipcMain.handle(
  "audio-get-frequency-state",
  (event: IpcMainEvent, frequency: number) => {
    return TrackAudioAfv.GetFrequencyState(frequency);
  }
);

ipcMain.handle("audio-is-frequency-active", (event: IpcMainEvent, frequency: number) => {
  return TrackAudioAfv.IsFrequencyActive(frequency);
});

ipcMain.handle("get-station", (event: IpcMainEvent, callsign: string) => {
  TrackAudioAfv.GetStation(callsign);
});

ipcMain.handle("refresh-station", (event: IpcMainEvent, callsign: string) => {
